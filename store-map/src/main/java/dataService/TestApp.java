...
import dataService.com.abbyy.ocrsdk.*;
...
public class TestApp {
	...
	public static void main(String path, String type, String language) {
		...
		argList.add("--lang=" + language);
		argList.add(path);
		argList.add(outPath.concat(".").concat(type));
	...
	private static boolean checkAppId() {
		...
		if (appId.equals("")|| password.equals("")) {
			System.out
	...
	private static ProcessingSettings.OutputFormat outputFormatByFileExt(
		...
		if (ext.equals(".txt")) {
			...
			return ProcessingSettings.OutputFormat.html;
		}
	    else {
			System.out
